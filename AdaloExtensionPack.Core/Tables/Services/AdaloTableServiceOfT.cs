...
public class AdaloTableService<T> : IAdaloTableService<T> where T : IAdaloEntity
{
    ...
    private static string GetTableUrl(Guid appId, string tableId, int? recordId = null,
        Expression<Func<T, bool>> predicate = null)
    {
        var url =
            $"https://api.adalo.com/v0/apps/{appId}/collections/{tableId}{(recordId != null ? "/" + recordId : "")}";

        var filter = predicate is
        {
            Body: BinaryExpression
            {
                NodeType: ExpressionType.Equal, Left: MemberExpression m,
                Right: ConstantExpression { Value: { } value }
            }
        }
            ? new Dictionary<string, string>
            {
                ["filterKey"] = GetMemberName(m),
                ["filterValue"] = value.ToString()
            }
            : default;

        return filter != default ? QueryHelpers.AddQueryString(url, filter) : url;
    }
